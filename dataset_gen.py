...
def generate_dataset(num_samples, selected_locations, file_path):
    ...
    for i in range(num_samples):
        ...
        smoker = random.choices([0, 1], weights=[random.uniform(85, 90), random.uniform(10, 15)])[0]  # 10-15% smokers
        ...
        asthma_diagnosed = random.choices([0, 1], weights=[90, 10])[0]  # 10% chance of being diagnosed with asthma
    ...
    df = pd.DataFrame(data, columns=['patient_id', 'age', 'gender', 'air_quality', 'living_conditions',
...
def save_dataset():
    ...
    generate_dataset(num_samples, selected_locations, file_path)
...
tk.Label(window, text="Number of Samples:").pack()
num_samples_entry = tk.Entry(window)
...
# Button to generate and save the dataset
